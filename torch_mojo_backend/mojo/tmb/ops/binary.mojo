...
Two families do the work: `logic` (the broadcast-strided binary and
ternary kernels) and `elementwise` (the contiguous tensor-with-scalar
...
def _b_bcast_dtype(st: Int32) -> Bool:
    """logic SPEC_BCAST_DTYPES (bool travels as uint8, see `_b_binary`)."""
    return (
        _b_float3(st)
        or st == ST_FLOAT64
        or st == ST_INT8
...
def _b_fillable(st: Int32) -> Bool:
    """elementwise SPEC_FILL_DTYPES (the old `_FILL_DTYPES`)."""
...
def _b_fill_spec(dst: T, value: Float64) raises:
    """elementwise FillSpec into a caller-allocated contiguous output."""
    var ctx = ctx_for(dst.device)
    var cp = ctx_ptr(ctx)
    var call = KernelCall("elementwise", "FillSpec")
...
def _b_binary_spec(op: StaticString, a: T, b: T, dst: T) raises:
    """logic broadcast binary into a caller-allocated contiguous output."""
    _one_device(a, dst)
    _one_device(b, dst)
    var ctx = ctx_for(dst.device)
    var cp = ctx_ptr(ctx)
...
def _b_scalar_spec(op: StaticString, a: T, value: Float64, dst: T) raises:
    """elementwise contiguous tensor-with-float-scalar."""
    _one_device(a, dst)
    var ctx = ctx_for(dst.device)
    var cp = ctx_ptr(ctx)
    var call = KernelCall("elementwise", String(op))
    call.arg_dtype(0, a.dtype)
    call.out_dtype(dst.dtype)
    call.spec(a.spec(cp))
    call.f64(value)
...
def _b_int_scalar_spec(op: StaticString, a: T, value: Int, dst: T) raises:
    """elementwise contiguous tensor-with-int-scalar."""
    _one_device(a, dst)
    var ctx = ctx_for(dst.device)
    var cp = ctx_ptr(ctx)
    var call = KernelCall("elementwise", String(op))
...
def _b_raw_add(dst: T, a: T, b: T) raises:
    """elementwise `Add`: contiguous, equal shapes, one dtype. `dst` may
    ...
    if a.numel == 0:
        return
    var ctx = ctx_for(dst.device)
    var call = KernelCall("elementwise", "Add")
    call.arg_dtype(0, a.dtype)
    call.arg_dtype(1, b.dtype)
    call.out_dtype(dst.dtype)
    call.int(dst.ptr)
...
def _b_binary(
    ...
    pre-materialization, output allocation and one logic launch.
...
def _b_div(lhs: Side, rhs: Side, mode: Value, dst: Optional[T]) raises -> Res:
    ...
    # data_movement' CAST_DTYPES stops before float64: `_b_cast` declines
...
def op_pow_scalar(args: Values, n_args: Int, rets: Values, n_rets: Int) raises:
    ...
    # dtype (logic SPEC_BCAST_DTYPES); `_b_binary` declines the rest.
