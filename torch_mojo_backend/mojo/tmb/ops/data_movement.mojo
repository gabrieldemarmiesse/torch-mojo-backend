...
def _is_scatter_dtype(dt: DType) -> Bool:
    """Mirrors tmb/kernels/data_movement/entry.mojo's `SCATTER_DTYPES` (no uint16/32/64).
    """
    return (
        dt == DType.float16
...
def _permute_materialize(t: T) raises -> T:
    ...
    var call = KernelCall("data_movement", "PermuteCopy")
...
def _cat_impl(ins: List[T], dim: Int) raises -> Owned:
    ...
    if ctx.api() != "cpu" and outer > 0 and dst_stride > 0 and all_contig:
        ...
        var cp = ctx_ptr(ctx)
        var call = KernelCall("data_movement", "CatN")
    ...
    for i in range(n):
        ...
        if copy_len > 0 and outer > 0:
            if b.contig:
                var cp2 = ctx_ptr(ctx)
                var call2 = KernelCall("data_movement", "NarrowCopyDst")
...
# Mirrors tmb/kernels/data_movement/entry.mojo's `_REPEAT_MAX_EXTENT`: both RepeatTiled
...
def op_repeat(args: Values, n_args: Int, rets: Values, n_rets: Int) raises:
    ...
    if out.t.numel > 0:
        ...
        if plan:
            ...
            var call = KernelCall("data_movement", "RepeatTiled")
            call.arg_dtype(0, t.dtype)
            call.out_dtype(t.dtype)
            call.int(out.t.ptr)
            call.int(src.ptr)
            ...
            var call = KernelCall("data_movement", "TileCopy")
            call.arg_dtype(0, t.dtype)
            call.out_dtype(t.dtype)
            call.int(out.t.ptr)
            call.int(src.ptr)
...
def _triangular(t: T, diagonal: Int, upper: Int) raises -> Owned:
    ...
    if out.t.numel > 0:
        ...
        var ctx = ctx_for(t.device)
        var cp = ctx_ptr(ctx)
        var call = KernelCall("data_movement", "TriangularCopy")
        call.arg_dtype(0, t.dtype)
        call.out_dtype(t.dtype)
        call.int(out.t.ptr)
        call.int(src.ptr)
...
def op_index_put_impl_(
    ...
    var validate = KernelCall("data_movement", "ScatterDim")
    ...
    if row_copy:
        ...
        for d in range(1, target.rank):
            rowlen *= target.dim(d)
        var rows = KernelCall("data_movement", "IndexPutRows")
