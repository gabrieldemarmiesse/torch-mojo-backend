...
from tmb.kernels.matmul.apple_gemm_nn_kernels import (
    apple_nn_direct_enqueue,
    apple_nn_smem_enqueue,
)
from tmb.kernels.matmul.apple_gemm_nt_kernels import (
    apple_nt_direct_enqueue,
    apple_nt_smem_enqueue,
)
from tmb.kernels.matmul.apple_gemm_tn_kernels import apple_tn_gemm_enqueue
from tmb.kernels.matmul.gemm_splitk_common import (
    TARGET_BLOCKS,
    _ksplit_reduce_kernel,
)
from tmb.kernels.matmul.tn_f32_gemm_kernels import try_enqueue_tn_f32_gemm
...
def _gemm_pipe_kernel[
    ...
    var acc = Array[SIMD[F32, TN], TM](fill=SIMD[F32, TN](0))
    var a_regs = Array[SIMD[F32, VEC_A], NA](uninitialized=True)
    var b_regs = Array[SIMD[F32, VEC_B], NB](uninitialized=True)
...
def _gemm_smallm_kernel[
    ...
    var acc = Array[Scalar[DType.float32], SMALLM_MR](
    ...
    for kt in range(k_start, k4, KU):
        var bv = Array[Scalar[DType.float32], KU](uninitialized=True)
...
def _apple8_gemm_kernel[
    ...
    for ks in range(ks0, ks1):
        var kk = ks * MMA8_DIM
        var afrag = Array[SIMD[DType.float32, FRAG8], NT_M](uninitialized=True)
        ...
        var bfrag = Array[SIMD[DType.float32, FRAG8], NT_N](uninitialized=True)
...
def _apple8_smem_kernel[
    ...
    def _compute(
        ...
        comptime for kc in range(BK // MMA8_DIM):
            var afrag = Array[SIMD[DType.float32, FRAG8], NT_M](
                uninitialized=True
            )
            comptime for mi in range(NT_M):
                afrag[mi] = (
            ...
            var bfrag = Array[SIMD[DType.float32, FRAG8], NT_N](
                uninitialized=True
            )
            comptime for ni in range(NT_N):
                bfrag[ni] = (
...
def _bmm_causal_go(
    ...
    comptime if has_apple_gpu_accelerator():
        if dtype != DType.float32 or causal_mode < 1 or causal_mode > 2:
