...
from tmb.kernels.fa4.fwd_common import (
    kFa4NThreads,
    kFa4BlockM,
    kFa4BlockN,
    kFa4NMmaWarpgroups,
...
@__llvm_metadata(
    ...
    `nvvm.minctasm`=SIMDLength(2 if kFa4NMmaWarpgroups(head_dim) == 1 else 1),
)
...
def fwd_fa4_kernel[
    ...
    if wgid == 0:
        ...
        if thread_idx.x == 0:
            mbar_q[unsafe_offset=0].expect_bytes(
            ...
            for _ in range(kv_trips):
                ...
                comptime if bhsd:
                    ...
                    k_tma.async_copy_3d(
                        k_st,
                        full[unsafe_offset=slot],
                        (0, h_idx // gqa_ratio, row),
                    )
                ...
                var v_st = LayoutTensor[
                    ...
                    (
                        kv_smem_base.unsafe_offset((slot + 1) * kv_slot_size)
                    ).as_unsafe_any_origin()
                )
