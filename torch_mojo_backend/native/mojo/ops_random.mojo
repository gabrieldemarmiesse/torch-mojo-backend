...
curand's (eager_kernels/random_ops, eager_kernels/dropout_ops). A tensor whose
...
from op_utils import MAX_RANK, _device_attr_cached
...
def _draw(
    ...
    for piece in pieces:
        ...
        var call = KernelCall("random_ops", String(op))
...
def op_bernoulli_tensor(
    ...
    var ctx = ctx_for(t.device)
    var cp = ctx_ptr(ctx)
    var call = KernelCall("random_ops", "BernoulliTensor")
    call.out_dtype(t.dtype)
...
def op_native_dropout(
    ...
    var cp = ctx_ptr(ctx)
    var call = KernelCall("dropout_ops", "NativeDropout")
...
def op_native_dropout_backward(
    ...
    if grad.numel > 0:
        ...
        var cp = ctx_ptr(ctx)
        var call = KernelCall("dropout_ops", "NativeDropoutBackward")
