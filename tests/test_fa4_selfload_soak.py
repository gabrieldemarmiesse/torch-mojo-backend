...
``tmb/kernels/fa4/fwd_selfload_kernel.mojo``).
...
kernel (``tmb/kernels/fa4/entry.mojo``'s bhsd entry points, above the wave gate; the
...
def test_selfload_bhsd_production_soak(shape: tuple[int, int, int], mojo_gpu: str):
    ...
    assert any("selfload" in name for name in kernels), (
        ...
        " in tmb/kernels/fa4/entry.mojo moved or this shape no longer reaches it"
    )
    ...
    for rep in range(_PROD_REPS):
        ...
        for index, out in enumerate(burst):
            assert torch.equal(out.cpu(), gold), (
                ...
                " its ring slot (see fwd_selfload_kernel.mojo's PREFETCH"
                " invariant)"
            )
